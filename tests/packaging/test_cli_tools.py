"""Tests for ``calfcord-package-tools``.

The CLI's docker-buildx invocation is NOT exercised here — that would
need Docker-in-Docker in CI. Tests use ``--dry-run`` to validate name
resolution + Dockerfile rendering without spinning up a build.
"""

from __future__ import annotations

import io
from contextlib import redirect_stderr, redirect_stdout

import pytest

from calfcord.packaging import cli_tools


def _run(argv: list[str]) -> tuple[int, str, str]:
    """Invoke ``cli_tools.main`` and capture stdout, stderr, exit code.

    Returns ``(exit_code, stdout, stderr)``. ``sys.exit`` from
    argparse / validation surfaces as ``SystemExit`` which we capture
    so the test can assert on its code.
    """
    stdout, stderr = io.StringIO(), io.StringIO()
    code = 0
    try:
        with redirect_stdout(stdout), redirect_stderr(stderr):
            code = cli_tools.main(argv)
    except SystemExit as e:
        code = e.code if isinstance(e.code, int) else 1
    return code, stdout.getvalue(), stderr.getvalue()


class TestDryRun:
    def test_emits_dockerfile_for_valid_tool(self) -> None:
        code, stdout, _ = _run(["terminal", "--tag", "test:1", "--dry-run"])
        assert code == 0
        # The header banner is the most stable structural marker —
        # if the Dockerfile rendered, this line is in the output.
        assert "Generated by calfcord-package-tools" in stdout
        assert "Hosts ONLY: terminal" in stdout

    def test_multiple_tools_in_one_image(self) -> None:
        code, stdout, _ = _run(["terminal", "search_files", "--tag", "test:1", "--dry-run"])
        assert code == 0
        # Both names baked into the env-var filter, alphabetically.
        assert "CALFCORD_TOOLS_INCLUDE=search_files,terminal" in stdout

    def test_duplicate_tool_name_deduped(self) -> None:
        code, stdout, _ = _run(["terminal", "terminal", "--tag", "test:1", "--dry-run"])
        assert code == 0
        # De-duplication: only one ``terminal`` appears in the include
        # CSV regardless of how many times the user typed it.
        assert "CALFCORD_TOOLS_INCLUDE=terminal\n" in stdout or \
               "CALFCORD_TOOLS_INCLUDE=terminal " in stdout


class TestNameValidation:
    def test_unknown_tool_exits_with_known_list(self) -> None:
        code, _, stderr = _run(["definitely_not_a_tool", "--tag", "x:1", "--dry-run"])
        assert code == 2
        assert "unknown tool" in stderr
        # Known list MUST be in the error so the operator can fix the
        # typo without grepping the source tree.
        assert "terminal" in stderr
        assert "private_chat" in stderr

    def test_one_unknown_among_valid_still_fails(self) -> None:
        code, _, stderr = _run(
            ["terminal", "typo_tool", "--tag", "x:1", "--dry-run"],
        )
        assert code == 2
        assert "typo_tool" in stderr

    @pytest.mark.parametrize("known_tool", ["terminal", "search_files", "read_file", "private_chat"])
    def test_known_tools_accepted(self, known_tool: str) -> None:
        code, _, _ = _run([known_tool, "--tag", "x:1", "--dry-run"])
        assert code == 0


class TestArgparseSurface:
    def test_help_shows_usage(self) -> None:
        # ``--help`` exits 0 via SystemExit; argparse writes to stdout.
        code, stdout, _ = _run(["--help"])
        assert code == 0
        assert "calfcord-package-tools" in stdout
        assert "--tag" in stdout
        assert "--dry-run" in stdout

    def test_missing_tag_required(self) -> None:
        code, _, stderr = _run(["terminal"])
        # argparse exits 2 on missing required arg.
        assert code == 2
        assert "--tag" in stderr

    def test_no_names_required(self) -> None:
        code, _, _ = _run(["--tag", "x:1"])
        assert code == 2  # argparse missing positional


class TestRenameFlag:
    """Tests for ``--rename SRC=DST``.

    Validation runs against the live ``TOOL_REGISTRY``, so these
    tests use real tool names (``patch``, ``terminal``) rather than
    fixture names. The CLI's dry-run mode lets us assert on the
    generated Dockerfile without invoking docker.
    """

    def test_rename_dry_run_bakes_env(self) -> None:
        """Happy path: ``--rename patch=patch_eu`` plus positional
        ``patch`` bakes both the alias env AND the post-rename
        include name into the image. The positional-name translation
        through the alias map is the load-bearing piece — without it,
        the filter would drop the clone and keep the original."""
        code, stdout, _ = _run(
            [
                "patch",
                "--rename",
                "patch=patch_eu",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 0
        assert "CALFCORD_TOOLS_ALIAS=patch=patch_eu" in stdout
        # Crucially the include filter references the POST-rename name,
        # otherwise discovery would drop the alias clone at boot.
        assert "CALFCORD_TOOLS_INCLUDE=patch_eu" in stdout

    def test_multiple_rename_flags(self) -> None:
        code, stdout, _ = _run(
            [
                "patch",
                "terminal",
                "--rename",
                "patch=patch_eu",
                "--rename",
                "terminal=terminal_eu",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 0
        # Sorted by src for determinism — pin the order so the build
        # cache hits across invocations with different flag ordering.
        assert "CALFCORD_TOOLS_ALIAS=patch=patch_eu,terminal=terminal_eu" in stdout
        # Both include names get translated.
        assert "patch_eu" in stdout
        assert "terminal_eu" in stdout

    def test_rename_unknown_source_exits_2(self) -> None:
        code, _, stderr = _run(
            [
                "terminal",
                "--rename",
                "nonexistent_tool=foo",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "nonexistent_tool" in stderr
        # Known list surfaces so the operator can fix the typo.
        assert "terminal" in stderr

    def test_rename_target_collides_with_existing_tool_exits_2(self) -> None:
        """Renaming ``patch`` to ``terminal`` (a real audited tool) would
        silently shadow ``terminal``. Refuse at build time."""
        code, _, stderr = _run(
            [
                "patch",
                "--rename",
                "patch=terminal",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "terminal" in stderr
        assert "collides" in stderr or "shadow" in stderr

    def test_rename_duplicate_target_exits_2(self) -> None:
        code, _, stderr = _run(
            [
                "patch",
                "terminal",
                "--rename",
                "patch=foo",
                "--rename",
                "terminal=foo",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "foo" in stderr
        assert "multiple" in stderr

    def test_rename_duplicate_source_exits_2(self) -> None:
        code, _, stderr = _run(
            [
                "patch",
                "--rename",
                "patch=a",
                "--rename",
                "patch=b",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "patch" in stderr
        assert "multiple" in stderr

    def test_rename_source_equals_target_exits_2(self) -> None:
        code, _, stderr = _run(
            [
                "patch",
                "--rename",
                "patch=patch",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "itself" in stderr or "distinct" in stderr

    def test_rename_malformed_no_equals_exits_2(self) -> None:
        code, _, stderr = _run(
            ["patch", "--rename", "patch", "--tag", "x:1", "--dry-run"]
        )
        assert code == 2
        assert "SRC=DST" in stderr

    def test_rename_empty_side_exits_2(self) -> None:
        code, _, stderr = _run(
            ["patch", "--rename", "patch=", "--tag", "x:1", "--dry-run"]
        )
        assert code == 2
        assert "empty" in stderr

    def test_rename_target_invalid_regex_exits_2(self) -> None:
        """The tool-name regex is Anthropic's: ``^[a-zA-Z0-9_-]{1,128}$``.
        Spaces fail. Catches typos like ``--rename foo='edit file_eu'``
        from a hand-edited terminal command."""
        code, _, stderr = _run(
            [
                "patch",
                "--rename",
                "patch=has spaces",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        assert "valid tool name" in stderr

    def test_rename_omitted_produces_no_alias_env(self) -> None:
        """Default behavior: no ``--rename`` flag → no
        ``CALFCORD_TOOLS_ALIAS`` line in the output. Defends against a
        regression that always bakes an empty alias env."""
        code, stdout, _ = _run(["terminal", "--tag", "x:1", "--dry-run"])
        assert code == 0
        assert "CALFCORD_TOOLS_ALIAS" not in stdout

    def test_rename_positional_mix_translates_only_renamed(self) -> None:
        """The most common operator pattern: host two tools, rename
        ONE of them. ``patch terminal --rename patch=patch_eu``
        should bake INCLUDE=``patch_eu,terminal`` (alphabetical CSV
        with only the renamed name translated; the un-renamed
        ``terminal`` passes through unchanged). The load-bearing line
        ``[aliases.get(n, n) for n in include_tools]`` would survive a
        regression that mistakenly maps non-renamed positionals
        through some default, but this test pins the contract that
        ONLY the renamed name gets translated."""
        code, stdout, _ = _run(
            [
                "patch",
                "terminal",
                "--rename",
                "patch=patch_eu",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 0
        # The CSV is sorted alphabetically — ``patch_eu`` comes
        # before ``terminal``. Pin the exact order so the build cache
        # hits across invocations.
        assert "CALFCORD_TOOLS_INCLUDE=patch_eu,terminal" in stdout
        # The alias only mentions the renamed tool; the un-renamed
        # ``terminal`` does NOT appear in the alias env.
        assert "CALFCORD_TOOLS_ALIAS=patch=patch_eu" in stdout
        assert "terminal=" not in stdout.split("CALFCORD_TOOLS_ALIAS=")[1].split("\n")[0]

    def test_rename_unused_alias_dead_config_exits_2(self) -> None:
        """``--rename patch=patch_eu`` WITHOUT ``patch`` in
        the positional list would bake a dead alias env into the
        image: ``CALFCORD_TOOLS_ALIAS`` is set but the include filter
        (the positional names) doesn't reference the post-rename
        name, so discovery would add the clone then drop it via the
        filter. Refuse at build time so operators don't ship images
        with rename env doing nothing."""
        code, _, stderr = _run(
            [
                "terminal",  # positional list does NOT include patch
                "--rename",
                "patch=patch_eu",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 2
        # The error must name the unused alias source explicitly so
        # the operator's diagnosis is one step.
        assert "patch" in stderr
        # "dead config" framing in the message — pins the operator-
        # comprehensible explanation that the diagnosis hinges on.
        assert "dead config" in stderr or "no effect" in stderr


class TestEnvStripping:
    """The CLI strips ``CALFCORD_TOOLS_ALIAS`` and
    ``CALFCORD_TOOLS_INCLUDE`` from its own environment BEFORE
    importing ``calfcord.tools``. Otherwise the operator's
    terminal env would poison the registry the CLI validates against —
    e.g. ``--rename anything=foo`` would silently pass because the
    operator's env already cloned ``foo``, and the resulting image
    would fail on a fresh host where the env isn't set.

    These tests exercise the strip-then-validate sequence by setting
    the env var and confirming the CLI behaves as if it weren't set.
    """

    def test_cli_strips_alias_env_before_validation(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """If the operator has ``CALFCORD_TOOLS_ALIAS`` set in their
        terminal when running ``calfcord-package-tools``, the CLI must
        ignore it. Verify by setting the env to something that WOULD
        change validation behavior if it leaked through — an alias
        targeting a name that doesn't exist in the canonical registry.
        With stripping working correctly, ``--rename patch=foo``
        succeeds (foo is unique); without stripping, the env would
        pre-populate foo and the CLI would erroneously reject the
        --rename as a collision."""
        # An env-only alias would, if not stripped, pre-populate
        # ``leaked_target`` in TOOL_REGISTRY before the CLI runs.
        monkeypatch.setenv("CALFCORD_TOOLS_ALIAS", "patch=leaked_target")
        # Now ask the CLI to alias patch to the SAME target the
        # leaked env was using. If stripping works, this should
        # succeed (leaked_target doesn't really exist in the
        # registry). If stripping fails, the CLI sees the leaked
        # registry and rejects as collision.
        code, stdout, _ = _run(
            [
                "patch",
                "--rename",
                "patch=leaked_target",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 0
        assert "CALFCORD_TOOLS_ALIAS=patch=leaked_target" in stdout

    def test_cli_strips_include_env_before_validation(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Symmetric to the alias-env strip: a leaked
        ``CALFCORD_TOOLS_INCLUDE`` would narrow the TOOL_REGISTRY the
        CLI imports, making the validation reject perfectly valid
        ``--rename`` sources as "not a known tool." Build-time CLI
        must validate against the FULL registry regardless of
        operator terminal state."""
        # Set the env to a NARROW filter that excludes patch.
        monkeypatch.setenv("CALFCORD_TOOLS_INCLUDE", "terminal")
        # If the env leaked through, patch wouldn't be in the
        # imported registry and --rename patch=foo would fail
        # with "not a known tool." Stripping makes this succeed.
        code, _, _ = _run(
            [
                "patch",
                "--rename",
                "patch=patch_eu",
                "--tag",
                "x:1",
                "--dry-run",
            ]
        )
        assert code == 0
